...
# Virtual Try-On Prompts
VTON_PROMPT_CN = """最终中文提示词（生产可用版）

执行图像编辑操作：删除并移除人物及所有配饰。
...
仅保留输入图像中真实可见、实际存在的核心服装品类（上衣、下装、外套）。
不得补充、生成、推断或添加任何未在输入图像中出现的服装品类。

仅允许对同一件已出现的服装进行合理补全，且补全范围严格限制在该服装自身的不可见或被遮挡区域。
严禁将单件服装补全为“整套穿搭”，不得引入任何新增服装。

服装必须以无人体支撑的形式展示（悬浮或平铺）。

服装颜色必须与输入图像 100% 完全一致。
不允许任何颜色变化、色相偏移、饱和度变化或亮度变化。
必须保留原始面料纹理、材质和图案。

背景必须为纯白色。

如检测到任何人体残留、配饰残留或输入图像中不存在的服装，必须继续删除，仅保留合法提取的服装内容。
"""
